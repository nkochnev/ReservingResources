...
let tryGetAccountFromContext ctx =
    ...
    | Some username ->
        let account =
            getAccounts() |> Seq.tryFind (fun u -> u.TelegramLogin = username)
        match account with
        | Some u -> Result.Ok u
        | None _ ->
            Result.Error(DomainEvent DomainEvents.AccountNotFoundByTelegramUser)
    | None _ ->
        Result.Error(TelegramEvent TelegramEvents.TelegramAccountIsEmpty)
